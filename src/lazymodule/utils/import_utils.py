...
# This class is copied from transformers.utils
class _LazyModule(ModuleType):
    """
    Module class that surfaces all objects but only performs associated imports when
    ...
    def __dir__(self):
        ...
        # The elements of self.__all__ that are submodules may or may not be in the dir
        # already, depending on whether they have been accessed or not. So we only add
        # the elements of self.__all__ that are not already in the dir.
    ...
    def _get_module(self, module_name: str):
        try:
            ...
            raise RuntimeError(
                ...
                f"following error (look up to see its "
                f"traceback):\n{e}"
            ) from e
